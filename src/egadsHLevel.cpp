  ...
  *mapping = map;
    ...
    if (listFaces.Extent() > 0) {
      ...
      for (; it.More(); it.Next()) {
        ...
        for (Exp.Init(result, TopAbs_SOLID); Exp.More(); Exp.Next()) {
          ...
          if (map[k] != NULL) 
            for (j = 0; j < nface; j++) 
              if (genface.IsSame(TopoDS::Face(MapF(j+1)))) map[k][j] = i;
          k++;
        }
      }
    }
  ...
  if (oface == NULL) {
    ...
    for (Exp.Init(result, TopAbs_SOLID); Exp.More(); Exp.Next()) {
      ...
      if (map[k] != NULL) 
        for (j = 0; j < nface; j++) 
          if (face.IsSame(TopoDS::Face(MapF(j+1)))) map[k][j] = -1;
      k++;
    }
    if (!BSO.IsDeleted(face)) {
      const TopTools_ListOfShape& listFaces = BSO.Modified(face);
      if (listFaces.Extent() > 0) {
        ...
        for (; it.More(); it.Next()) {
          ...
          for (Exp.Init(result, TopAbs_SOLID); Exp.More(); Exp.Next()) {
            ...
            if (map[k] != NULL) 
              for (j = 0; j < nface; j++) 
                if (genface.IsSame(TopoDS::Face(MapF(j+1)))) map[k][j] = -1;
            k++;
          }
        }
      }
...
EG_loft(int nsec, const egObject **secs, int opt, egObject **result)
{
  ...
  *result = NULL;
  ...
  TopoDS_Shape newShape = loft.Shape();
