    ...
    for (itri = 1; itri <= btess->tess2d[iface-1].ntris; itri++) {
      for (j = 0; j < 3; j++) {
        ...
        if (n > btess->tess2d[iface-1].ntris) {
          printf(" checkTriangulation: Face %d, Nei %d[%d] = %d!\n",
                 iface, itri, j, n);
        } else if (n == 0) {
          ...
          if ((ie < 1) || (ie > btess->nEdge)) {
            ...
            if (iv == 0) {
              for (i = 0; i <  btess->tess1d[ie-1].npts-1; 
                ...
                if (btess->tess1d[ie-1].faces[0] == iface)
                  if (btess->tess1d[ie-1].tric[2*i  ] == itri) break;
                if (btess->tess1d[ie-1].faces[1] == iface)
                  if (btess->tess1d[ie-1].tric[2*i+1] == itri) break;
              }
              ...
              i = 0;
              if ((btess->tess1d[ie-1].faces[0] == iface) &&
                  (btess->tess1d[ie-1].tric[2*iv-2] == itri)) i++;
              if ((btess->tess1d[ie-1].faces[1] == iface) &&
                  (btess->tess1d[ie-1].tric[2*iv-1] == itri)) i++;
              if (i == 0) {
                printf(" checkTriangulation: Face %d, Tri Nei %d[%d] Edge %d =",
                       iface, itri, j, ie);
                if (btess->tess1d[ie-1].faces[0] == iface)
                  printf(" %d", btess->tess1d[ie-1].tric[2*iv-2]);
                if (btess->tess1d[ie-1].faces[1] == iface)
                  printf(" %d", btess->tess1d[ie-1].tric[2*iv-1]);
                printf("!\n");
              }
            }
          }
...
{
  int    i, j, k, n, edge, *ptype, *pindex, *tris, *tric;
  double *xyz, *uv;
    ...
    for (j = 0; j < 3; j++) 
      if (tric[3*i+j] < 0) {
        ...
        if (ts->segs[n-1].edge > 0) {
          btess->tess1d[edge-1].tric[2*k+1] = i+1;
        } else {
          btess->tess1d[edge-1].tric[2*k  ] = i+1;
        }
        tric[3*i+j] = -edge;
      }
...
EG_cleanupTess(egTessel *btess)
{
  ...
  if (btess->tess1d != NULL) {
    for (i = 0; i < btess->nEdge; i++) {
      if (btess->tess1d[i].tric != NULL) 
        EG_free(btess->tess1d[i].tric);
...
EG_tessEdges(egTessel *btess)
{
  ...
  double   xyz[MAXELEN][3], t[MAXELEN], aux[MAXELEN][3], uv[2], mindist, tol;
  double   d, dist, dotnrm, dot, area, limits[2], mid[3], range[4];
  double   x1[3], x2[3], nrme[3], nrmi[3], dx[3], ds[3], result[18];
  ...
  for (j = 0; j < nedge; j++) {
      ...
      for (n = 0; n < 2; n++) {
        ...
        while (npts < MAXELEN) {
          ...
          for (i = 0; i < npts-1; i++) {
            ...
            stat  = EG_evaluate(faces[face-1], uv, result);
            if (stat != EGADS_SUCCESS) {
              aux[i][2] = -1.0;
              continue;
            }
            ...
            stat  = EG_invEvaluate(faces[face-1], x1, uv, x2);
            if (stat != EGADS_SUCCESS) {
              aux[i][2] = -1.0;
              continue;
            }
...
#ifdef DEBUG
        printf("     FacNrm Phase npts = %d @ %lf (%lf)  Face = %d\n",
               npts, dot, dotnrm, face);
#endif
      }
    ...
    if (btess->params[0] > 0.0)
      while (npts < MAXELEN) {
        ...
        k    = 0;
        dist = (xyz[0][0]-xyz[1][0])*(xyz[0][0]-xyz[1][0]) +
               (xyz[0][1]-xyz[1][1])*(xyz[0][1]-xyz[1][1]) +
               (xyz[0][2]-xyz[1][2])*(xyz[0][2]-xyz[1][2]);
        for (i = 1; i < npts-1; i++) {
          d = (xyz[i][0]-xyz[i+1][0])*(xyz[i][0]-xyz[i+1][0]) +
              (xyz[i][1]-xyz[i+1][1])*(xyz[i][1]-xyz[i+1][1]) +
              (xyz[i][2]-xyz[i+1][2])*(xyz[i][2]-xyz[i+1][2]);
          if (d > dist) {
            dist = d;
    ...
    btess->tess1d[j].tric = (int *) EG_alloc((2*npts-2)*sizeof(int));
    if (btess->tess1d[j].tric == NULL) {
      EG_free(btess->tess1d[j].xyz);
      btess->tess1d[j].xyz = NULL;
      if (outLevel > 0)
...
{
  int      i, m, stat, outLevel, nedge, nface, iface, itri, ivrt;
  int      sense;
  double   result[9], uvs[2][2];
  egTessel *btess;
  ...
  iface = btess->tess1d[eIndex-1].faces[0];
  if (iface != 0) {
    sense = faces[iface-1]->mtype;
    if (btess->tess1d[eIndex-1].faces[0] !=
        btess->tess1d[eIndex-1].faces[1]) sense = 0;
  ...
  iface = btess->tess1d[eIndex-1].faces[1];
  if (iface != 0) {
    sense = faces[iface-1]->mtype;
    if (btess->tess1d[eIndex-1].faces[0] !=
        btess->tess1d[eIndex-1].faces[1]) sense = 0;
  ...
  for (m = 0; m < 2; m++) {
    iface = btess->tess1d[eIndex-1].faces[m];
    if (iface == 0) continue;
    itri  = btess->tess1d[eIndex-1].tric[2*vIndex+m-2] - 1;
    for (i = 0; i < 3; i++) {
      ...
      if ((btess->tess2d[iface-1].pindex[ivrt] == eIndex) &&
        ...
        btess->tess2d[iface-1].uv[2*ivrt  ]  = uvs[m][0];
        btess->tess2d[iface-1].uv[2*ivrt+1]  = uvs[m][1];
        break;
      }
    }
    /* delete any quads */
...
{
  int      i, j, k, m, outLevel, iface, iv[4], it[4], ivert;
  int      n1, n2, ie, i1, i2, i3, pt1, pi1, pt2, pi2;
  egTessel *btess;
  egObject *obj;
  ...
  if (dir == -1) {
    it[0] = btess->tess1d[eIndex-1].tric[2*vIndex-2];
    it[1] = btess->tess1d[eIndex-1].tric[2*vIndex-1];
    it[2] = btess->tess1d[eIndex-1].tric[2*vIndex  ];
    it[3] = btess->tess1d[eIndex-1].tric[2*vIndex+1];
  } else {
    it[0] = btess->tess1d[eIndex-1].tric[2*vIndex  ];
    it[1] = btess->tess1d[eIndex-1].tric[2*vIndex+1];
    it[2] = btess->tess1d[eIndex-1].tric[2*vIndex-2];
    it[3] = btess->tess1d[eIndex-1].tric[2*vIndex-1];
  }
  for (m = 0; m < 2; m++) {
    iface = btess->tess1d[eIndex-1].faces[m];
    ...
    for (i = 0; i < 3; i++) {
      ivert = btess->tess2d[iface-1].tris[3*it[m]+i-3] - 1;
      if ((btess->tess2d[iface-1].pindex[ivert] == eIndex) &&
          (btess->tess2d[iface-1].ptype[ivert]  == vIndex)) {
        iv[m] = ivert;
        break;
      }
    }
    ...
    for (i = 0; i < 3; i++) {
      ivert = btess->tess2d[iface-1].tris[3*it[m+2]+i-3] - 1;
      if ((btess->tess2d[iface-1].pindex[ivert] == eIndex) &&
          (btess->tess2d[iface-1].ptype[ivert]  == vIndex+dir)) {
        iv[m+2] = ivert;
        break;
      }
    }
    ...
    i1 = btess->tess2d[iface-1].tris[3*it[m]-3]-1;
    i2 = btess->tess2d[iface-1].tris[3*it[m]-2]-1;
    i3 = btess->tess2d[iface-1].tris[3*it[m]-1]-1;        
    if (((btess->tess2d[iface-1].pindex[i2] == pi1) &&
      ...
      n1 = btess->tess2d[iface-1].tric[3*it[m]-2];
      n2 = btess->tess2d[iface-1].tric[3*it[m]-1];
               ...
               ((btess->tess2d[iface-1].pindex[i1] == pi2) &&
                ...
                (btess->tess2d[iface-1].pindex[i3] == pi1) &&
                (btess->tess2d[iface-1].ptype[i3]  == pt1))) {
      n1 = btess->tess2d[iface-1].tric[3*it[m]-3];
      n2 = btess->tess2d[iface-1].tric[3*it[m]-1];
    } else if (((btess->tess2d[iface-1].pindex[i1] == pi1) &&
                (btess->tess2d[iface-1].ptype[i1]  == pt1) &&
                (btess->tess2d[iface-1].pindex[i2] == pi2) &&
               ...
               ((btess->tess2d[iface-1].pindex[i1] == pi2) &&
                ...
                (btess->tess2d[iface-1].ptype[i2]  == pt1))) {
      n1 = btess->tess2d[iface-1].tric[3*it[m]-3];
      n2 = btess->tess2d[iface-1].tric[3*it[m]-2];
    } else {
    ...
    if (n1 > 0) {
      for (i = 0; i < 3; i++)
        if (btess->tess2d[iface-1].tric[3*n1+i-3] == it[m]) {
      ...
      ie = -n1;
      for (j = 0; j < 2; j++) {
        if (btess->tess1d[ie-1].faces[j] != iface) continue;
        for (k = 0; k < btess->tess1d[ie-1].npts-1; k++)
          if (btess->tess1d[ie-1].tric[2*k+j] == it[m])
            btess->tess1d[ie-1].tric[2*k+j] = n2;
      }
    }
    if (n2 > 0) {
      for (i = 0; i < 3; i++)
        if (btess->tess2d[iface-1].tric[3*n2+i-3] == it[m]) {
      ...
      for (j = 0; j < 2; j++) {
        ...
        for (k = 0; k < btess->tess1d[ie-1].npts-1; k++)
          if (btess->tess1d[ie-1].tric[2*k+j] == it[m])
            btess->tess1d[ie-1].tric[2*k+j] = n1;
      }
    }
    /* collapse the vert from the triangulation by subsitution*/
    for (i = 0; i < btess->tess2d[iface-1].ntris; i++) {
      if (btess->tess2d[iface-1].tris[3*i  ] == iv[m])
        btess->tess2d[iface-1].tris[3*i  ] = iv[m+2];
      if (btess->tess2d[iface-1].tris[3*i+1] == iv[m])
        btess->tess2d[iface-1].tris[3*i+1] = iv[m+2];
      if (btess->tess2d[iface-1].tris[3*i+2] == iv[m])
        btess->tess2d[iface-1].tris[3*i+2] = iv[m+2];
    }
  }

  /* compress the Edge storage */
  btess->tess1d[eIndex-1].npts--;
  for (i = vIndex-1; i < btess->tess1d[eIndex-1].npts; i++) {
    btess->tess1d[eIndex-1].tric[2*i  ] = btess->tess1d[eIndex-1].tric[2*i+2];
    btess->tess1d[eIndex-1].tric[2*i+1] = btess->tess1d[eIndex-1].tric[2*i+3];
  ...
  for (m = 0; m < 2; m++) {
    if (iv[m] == -1) continue;
    iface = btess->tess1d[eIndex-1].faces[m];
    if ((m == 0) || (btess->tess1d[eIndex-1].faces[0] !=
                     btess->tess1d[eIndex-1].faces[1]))
    ...
    for (i = 0; i < btess->tess2d[iface-1].ntris; i++) {
      ...
      if (btess->tess2d[iface-1].tric[3*i  ] > it[m])
        btess->tess2d[iface-1].tric[3*i  ]--;
      if (btess->tess2d[iface-1].tric[3*i+1] > it[m])
        btess->tess2d[iface-1].tric[3*i+1]--;
      if (btess->tess2d[iface-1].tric[3*i+2] > it[m])
        btess->tess2d[iface-1].tric[3*i+2]--;
    }
    btess->tess2d[iface-1].npts--;
    for (i = iv[m]; i < btess->tess2d[iface-1].npts; i++) {
    ...
    for (i = it[m]; i < btess->tess2d[iface-1].ntris; i++) {
...
EG_insertEdgeVerts(egObject *tess, int eIndex, int vIndex, int npts,
                   double *t)
{
  int      i, j, k, m, mx, n, stat, outLevel, nedge, nface, iface, itri;
  int      n0, n1, v0, v1, vert, vn, nl, nn, sense, pt1, pi1, pt2, pi2;
  int      i1, i2, i3;
  int      *etric, *pindex[2], *ptype[2], *tris[2], *tric[2];
  double   result[9], *vals, *xyzs, *ts, *xyz[2], *uv[2];
  egTessel *btess;
  egObject *obj, **edges, **faces;
  ...
  for (mx = 0; mx < 2; mx++) {
    iface = btess->tess1d[eIndex-1].faces[mx];
    if (iface == 0) continue;
    itri = btess->tess1d[eIndex-1].tric[2*vIndex+mx-2];
  ...
  vals = (double *) EG_alloc(7*npts*sizeof(double));
  if (vals == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on Tmp %d (EG_insertEdgeVerts)!\n",
             npts);
    return EGADS_MALLOC;
  }
  stat = EG_getBodyTopos(btess->src, NULL, EDGE, &nedge, &edges);
  ...
  /* correct for closed face 
  if (btess->tess1d[eIndex-1].faces[0] == 
      btess->tess1d[eIndex-1].faces[1]) {
    itri = btess->tess1d[eIndex-1].tric[0]-1;
    if (btess->tess2d[iface-1].tric[3*itri  ] == -eIndex) {
      i1 = btess->tess2d[iface-1].tris[3*itri+1]-1;
      i2 = btess->tess2d[iface-1].tris[3*itri+2]-1;
    }
    if (btess->tess2d[iface-1].tric[3*itri+1] == -eIndex) {
      i1 = btess->tess2d[iface-1].tris[3*itri  ]-1;
      i2 = btess->tess2d[iface-1].tris[3*itri+2]-1;
    } 
    if (btess->tess2d[iface-1].tric[3*itri+2] == -eIndex) {
      i1 = btess->tess2d[iface-1].tris[3*itri  ]-1;
      i2 = btess->tess2d[iface-1].tris[3*itri+1]-1;
    }
    sense = -1;
    if (fabs(btess->tess2d[iface-1].uv[2*i2  ]-
             btess->tess2d[iface-1].uv[2*i1  ]) < 1.e-6) sense = 0;
    if (fabs(btess->tess2d[iface-1].uv[2*i2+1]-
             btess->tess2d[iface-1].uv[2*i1+1]) < 1.e-6) sense = 1;
    if (outLevel > 2)
      printf(" neg = %lf %lf  %lf %lf   %d\n", 
             btess->tess2d[iface-1].uv[2*i1  ], 
             btess->tess2d[iface-1].uv[2*i1+1],
             btess->tess2d[iface-1].uv[2*i2  ], 
             btess->tess2d[iface-1].uv[2*i2+1], sense);
    if (sense == 0)
      if (fabs(btess->tess2d[iface-1].uv[2*i2  ]-vals[5]) < 1.e-6)
        for (i = 0; i < npts; i++) {
          save        = vals[7*i+3];
          vals[7*i+3] = vals[7*i+5];
          vals[7*i+5] = save;
          save        = vals[7*i+4];
          vals[7*i+4] = vals[7*i+6];
          vals[7*i+6] = save;
        }
    if (sense == 1)
      if (fabs(btess->tess2d[iface-1].uv[2*i2+1]-vals[6]) < 1.e-6)
        for (i = 0; i < npts; i++) {
          save        = vals[7*i+3];
          vals[7*i+3] = vals[7*i+5];
          vals[7*i+5] = save;
  ...
  for (j = i = 0; i < btess->tess1d[eIndex-1].npts; i++, j++) {
    ...
    if (i != btess->tess1d[eIndex-1].npts-1) {
      etric[2*j  ] = btess->tess1d[eIndex-1].tric[2*i  ];
      etric[2*j+1] = btess->tess1d[eIndex-1].tric[2*i+1];
    }
    if (i != vIndex-1) continue;
    for (k = 0; k < npts; k++) {
      j++;
      xyzs[3*j  ]  = vals[7*k  ];
      xyzs[3*j+1]  = vals[7*k+1];
      xyzs[3*j+2]  = vals[7*k+2];
      ts[j]        = t[k];
      etric[2*j  ] = 0;
      etric[2*j+1] = 0;
    }
  }
  /* allocate for the Faces */
  tris[0] = tric[0] = pindex[0] = ptype[0] = NULL;
  xyz[0]  = uv[0]   = NULL;
  tris[1] = tric[1] = pindex[1] = ptype[1] = NULL;
  xyz[1]  = uv[1]   = NULL;
  for (m = 0; m < 2; m++) {
    iface = btess->tess1d[eIndex-1].faces[m];
    if (iface == 0) continue;
    n = 1;
    if ((m == 0) && (iface == btess->tess1d[eIndex-1].faces[1])) n = 2;
    xyz[m]    = (double *) EG_alloc(3*(n*npts+btess->tess2d[iface-1].npts)*
                                    sizeof(double));
    uv[m]     = (double *) EG_alloc(2*(n*npts+btess->tess2d[iface-1].npts)*
                                    sizeof(double));
    ptype[m]  = (int *)    EG_alloc(  (n*npts+btess->tess2d[iface-1].npts)*
                                    sizeof(int));
    pindex[m] = (int *)    EG_alloc(  (n*npts+btess->tess2d[iface-1].npts)*
                                    sizeof(int));
    tris[m]   = (int *)    EG_alloc(3*(n*npts+btess->tess2d[iface-1].ntris)*
                                    sizeof(int));
    tric[m]   = (int *)    EG_alloc(3*(n*npts+btess->tess2d[iface-1].ntris)*
                                    sizeof(int));
    if ((xyz[m] == NULL) || (uv[m] == NULL) || (ptype[m] == NULL) ||
        (pindex[m] == NULL) || (tris[m] == NULL) || (tric[m] == NULL)) {
      if (outLevel > 0)
        printf(" EGADS Error: Malloc on Edge %d %d(EG_insertEdgeVerts)!\n",
               npts, btess->tess1d[eIndex-1].npts);
      if (tric[1]   != NULL) EG_free(tric[1]);
      if (tris[1]   != NULL) EG_free(tris[1]);
      if (pindex[1] != NULL) EG_free(pindex[1]);
      if (ptype[1]  != NULL) EG_free(ptype[1]);
      if (uv[1]     != NULL) EG_free(uv[1]);
      if (xyz[1]    != NULL) EG_free(xyz[1]);
      if (tric[0]   != NULL) EG_free(tric[0]);
      if (tris[0]   != NULL) EG_free(tris[0]);
      if (pindex[0] != NULL) EG_free(pindex[0]);
      if (ptype[0]  != NULL) EG_free(ptype[0]);
      if (uv[0]     != NULL) EG_free(uv[0]);
      if (xyz[0]    != NULL) EG_free(xyz[0]);
      EG_free(etric);
      EG_free(ts);
      EG_free(xyzs);
      EG_free(vals);
    ...
    for (i = 0; i < btess->tess2d[iface-1].npts; i++) {
      xyz[m][3*i  ] = btess->tess2d[iface-1].xyz[3*i  ];
      xyz[m][3*i+1] = btess->tess2d[iface-1].xyz[3*i+1];
      xyz[m][3*i+2] = btess->tess2d[iface-1].xyz[3*i+2];
      uv[m][2*i  ]  = btess->tess2d[iface-1].uv[2*i  ];
      uv[m][2*i+1]  = btess->tess2d[iface-1].uv[2*i+1];
      ptype[m][i]   = btess->tess2d[iface-1].ptype[i];
      pindex[m][i]  = btess->tess2d[iface-1].pindex[i];
      if (pindex[m][i] == eIndex) 
        if (ptype[m][i] > vIndex) ptype[m][i] += npts;
    }
    j = btess->tess2d[iface-1].npts;
    for (i = 0; i < npts; i++) {
      xyz[m][3*(j+i)  ] = vals[7*i  ];
      xyz[m][3*(j+i)+1] = vals[7*i+1];
      xyz[m][3*(j+i)+2] = vals[7*i+2];
      uv[m][2*(i+j)  ]  = vals[7*i+3+2*m  ];
      uv[m][2*(i+j)+1]  = vals[7*i+3+2*m+1];
      ptype[m][i+j]     = vIndex + i+1;
      pindex[m][i+j]    = eIndex;
    }
    j = btess->tess2d[iface-1].npts + npts;
    if (n == 2)
      for (i = 0; i < npts; i++) {
        xyz[m][3*(j+i)  ] = vals[7*i  ];
        xyz[m][3*(j+i)+1] = vals[7*i+1];
        xyz[m][3*(j+i)+2] = vals[7*i+2];
        uv[m][2*(i+j)  ]  = vals[7*i+5];
        uv[m][2*(i+j)+1]  = vals[7*i+6];
        ptype[m][i+j]     = vIndex + i+1;
        pindex[m][i+j]    = eIndex;
      }    
    for (i = 0; i < btess->tess2d[iface-1].ntris; i++) {
      tris[m][3*i  ] = btess->tess2d[iface-1].tris[3*i  ];
      tris[m][3*i+1] = btess->tess2d[iface-1].tris[3*i+1];
      tris[m][3*i+2] = btess->tess2d[iface-1].tris[3*i+2];
      tric[m][3*i  ] = btess->tess2d[iface-1].tric[3*i  ];
      tric[m][3*i+1] = btess->tess2d[iface-1].tric[3*i+1];
      tric[m][3*i+2] = btess->tess2d[iface-1].tric[3*i+2];
    }
    if (n == 2) break;
  }
  EG_free(vals);
  ...
  if (btess->tess1d[eIndex-1].tric != NULL) 
    EG_free(btess->tess1d[eIndex-1].tric);
  ...
  for (mx = 0; mx < 2; mx++) {
    ...
    n = 0;
    m = mx;
    if ((m == 1) && (iface == btess->tess1d[eIndex-1].faces[0])) {
      n = npts;
      m = 0;
    }
    sense = 1;
    ...
    i1 = tris[m][3*itri-3]-1;
    i2 = tris[m][3*itri-2]-1;
    i3 = tris[m][3*itri-1]-1;        
    if (((btess->tess2d[iface-1].pindex[i2] == pi1) &&
         (btess->tess2d[iface-1].ptype[i2]  == pt1) &&
         (btess->tess2d[iface-1].pindex[i3] == pi2) &&
         (btess->tess2d[iface-1].ptype[i3]  == pt2)) || 
...
/*  printf("       %d: %d", iface, itri); 
    ...
    for (i = 0; i < 3; i++) {
      if (btess->tess2d[iface-1].tris[3*itri+i-3] == v1) 
        tris[m][3*itri+i-3] = btess->tess2d[iface-1].npts  + n + 1;
      if (btess->tess2d[iface-1].tris[3*itri+i-3] == v0) 
        tric[m][3*itri+i-3] = btess->tess2d[iface-1].ntris + n + 1;
    } 
    nl = itri;
    for (i = 0; i < npts; i++) {
      j  = btess->tess2d[iface-1].ntris + i + n;
      v0 = btess->tess2d[iface-1].npts  + i + n + 1;
      vn = btess->tess2d[iface-1].npts  + i + n + 2;
      nn = j+2;
      if (i == npts-1) {
        vn = v1;
        nn = n0;
      ...
      btess->tess1d[eIndex-1].tric[2*(vIndex+i)+mx] = j + 1;
      nl = j+1;
    }
    if (n0 > 0) {
      for (i = 0; i < 3; i++)
        if (btess->tess2d[iface-1].tric[3*n0+i-3] == itri) 
          tric[m][3*n0+i-3] = btess->tess2d[iface-1].ntris+n+npts;
    } else if (n0 < 0) {
      j = -n0 - 1;
      if (btess->tess1d[j].faces[0] == iface) {
        for (k = i = 0; i < btess->tess1d[j].npts-1; i++)
          if (btess->tess1d[j].tric[2*i  ] == itri) k++;
        for (i = 0; i < btess->tess1d[j].npts-1; i++) {
          if ((k > 1) && (i >= vIndex-1) && (i < vIndex+npts-1)) continue;
          if (btess->tess1d[j].tric[2*i  ] == itri)
            btess->tess1d[j].tric[2*i  ] = 
              btess->tess2d[iface-1].ntris+n+npts;
        }
      }
      if (btess->tess1d[j].faces[1] == iface) {
        for (k = i = 0; i < btess->tess1d[j].npts-1; i++)
          if (btess->tess1d[j].tric[2*i+1] == itri) k++;
        for (i = 0; i < btess->tess1d[j].npts-1; i++) {
          if ((k > 1) && (i >= vIndex-1) && (i < vIndex+npts-1)) continue;
          if (btess->tess1d[j].tric[2*i+1] == itri) 
            btess->tess1d[j].tric[2*i+1] = 
              btess->tess2d[iface-1].ntris+n+npts;
        }
      }
    }
  ...
  for (m = 0; m < 2; m++) {
    ...
    btess->tess2d[iface-1].xyz    = xyz[m];
    btess->tess2d[iface-1].uv     = uv[m];
    btess->tess2d[iface-1].ptype  = ptype[m];
    btess->tess2d[iface-1].pindex = pindex[m];
    btess->tess2d[iface-1].tris   = tris[m];
    btess->tess2d[iface-1].tric   = tric[m];
    btess->tess2d[iface-1].ntris += n*npts;
    btess->tess2d[iface-1].npts  += n*npts;
